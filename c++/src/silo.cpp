    ...
    try
    {
        ...
        if (arg == "concurrent")
        {
            ...
            std::unique_ptr<queue::NonBlockingQueue<turning_grille::Grille>> nonBlockingQueue =
            		std::make_unique<queue::ConcurrentPortionQueue<turning_grille::Grille>>(maxQueueSize);
            std::unique_ptr<queue::MPMC_PortionQueue<turning_grille::Grille>> portionQueue =
            		std::make_unique<queue::MostlyNonBlockingPortionQueue<turning_grille::Grille>>(maxQueueSize, std::move(nonBlockingQueue));
        ...
        else if (arg == "atomic")
        {
            ...
            std::unique_ptr<queue::NonBlockingQueue<turning_grille::Grille>> nonBlockingQueue =
            		std::make_unique<queue::AtomicPortionQueue<turning_grille::Grille>>(maxQueueSize);
            std::unique_ptr<queue::MPMC_PortionQueue<turning_grille::Grille>> portionQueue =
            		std::make_unique<queue::MostlyNonBlockingPortionQueue<turning_grille::Grille>>(maxQueueSize, std::move(nonBlockingQueue));
            crackerImplDetails = std::make_unique<turning_grille::TurningGrilleCrackerProducerConsumer>(initialConsumerCount, producerCount, std::move(portionQueue));
        }
        else if (arg == "lockfree")
        {
            ...
            std::unique_ptr<queue::NonBlockingQueue<turning_grille::Grille>> nonBlockingQueue =
            		std::make_unique<queue::LockfreePortionQueue<turning_grille::Grille>>(maxQueueSize);
            std::unique_ptr<queue::MPMC_PortionQueue<turning_grille::Grille>> portionQueue =
            		std::make_unique<queue::MostlyNonBlockingPortionQueue<turning_grille::Grille>>(maxQueueSize, std::move(nonBlockingQueue));
            crackerImplDetails = std::make_unique<turning_grille::TurningGrilleCrackerProducerConsumer>(initialConsumerCount, producerCount, std::move(portionQueue));
        }
